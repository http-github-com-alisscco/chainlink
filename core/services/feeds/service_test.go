...
func Test_Service_RejectJobProposal(t *testing.T) {
	...
	ctx = mockTransact(svc.txm)
	svc.orm.On("UpdateJobProposalStatus", ctx, jp.ID, feeds.JobProposalStatusRejected).Return(nil)
	svc.fmsClient.On("RejectedJob", ctx, &proto.RejectedJobRequest{
		Uuid: jp.UUID.String(),
	}).Return(&proto.RejectedJobResponse{}, nil)

	err := svc.RejectJobProposal(jp.ID)
	require.NoError(t, err)
}
...
func mockTransact(txm *pgmocks.TransactionManager) context.Context {
	ctx := context.Background()
	call := txm.On("Transact", mock.Anything)
	call.Run(func(args mock.Arguments) {
		arg := args.Get(0).(postgres.TxFn)
		err := arg(ctx)
		call.Return(err)
	})
