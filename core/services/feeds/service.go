...
type Service interface {
	...
	RejectJobProposal(id int64) error
...
func (s *service) RejectJobProposal(id int64) error {
	if s.fmsClient == nil {
	...
	err = s.txm.Transact(func(ctx context.Context) error {
		if err = s.orm.UpdateJobProposalStatus(ctx, id, JobProposalStatusRejected); err != nil {
			return err
		}

		if _, err = s.fmsClient.RejectedJob(context.Background(), &pb.RejectedJobRequest{
			Uuid: jp.UUID.String(),
		}); err != nil {
			return err
		}
