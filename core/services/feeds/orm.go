...
func (o *orm) CreateJobProposal(ctx context.Context, jp *JobProposal) (int64, error) {
	...
	stmt := `
		INSERT INTO job_proposals (uuid, spec, status, feeds_manager_id, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?)
		RETURNING id;
	`

	row := o.db.Raw(stmt, jp.UUID, jp.Spec, jp.Status, jp.FeedsManagerID, now, now).Row()
	if row.Err() != nil {
		return id, row.Err()
	}
...
func (o *orm) ListJobProposals(ctx context.Context) ([]JobProposal, error) {
	jps := []JobProposal{}
	stmt := `
		SELECT uuid, id, spec, status, job_id, feeds_manager_id, created_at, updated_at
		FROM job_proposals;
	`
...
func (o *orm) GetJobProposal(ctx context.Context, id int64) (*JobProposal, error) {
	stmt := `
		SELECT id, uuid, spec, status, job_id, feeds_manager_id, created_at, updated_at
		FROM job_proposals
		WHERE id = ?;
	`
